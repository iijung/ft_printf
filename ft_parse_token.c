...
/*   Updated: 2022/08/08 10:24:16 by minjungk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (s == 0 && token->opt.length < (int)ft_strlen("(null)"))
		token->opt.length = 0;
	else if (s == 0)
	{
		s = "(null)";
...
static int	parse_puxX(t_token *token, unsigned long ul)
{
	...
	if (token->opt.found && token->opt.minus)
		ft_memcpy(token->out, "0x", 2);
	if (token->opt.found && token->opt.minus == 0)
